...
//
//raycirclelist intersect
//point = startpoint
int closestNode(Vec2 point, Vec2[] nodePos, int numNodes,Vec2[] centers, float[] radii){
  int closestID = -1;
  float minDist = 999999; //set to infinity
  for (int i = 0; i < numNodes; i++){
...
ArrayList<Integer> planPath(Vec2 startPos, Vec2 goalPos, Vec2[] centers, float[] radii, int numObstacles, Vec2[] nodePos, int numNodes){
  ...
  int startID = closestNode(startPos, nodePos, numNodes, centers, radii);
  int goalID = closestNode(goalPos, nodePos, numNodes, centers, radii);
